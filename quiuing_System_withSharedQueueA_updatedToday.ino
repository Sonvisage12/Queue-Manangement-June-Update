...
void onDataRecv(const esp_now_recv_info_t *recvInfo, const uint8_t *incomingData, int len) {
    ...
    if (isArrivalNode) {
        ...
        if (strcmp(item.uid, "REQ_NEXT") == 0) {
            ...
            if (doctorNodeID == -1) { Serial.println("❌ Unknown Doctor Node MAC!"); return; }
            ...
            Serial.printf("➕ Moved UID %s to Queue A.\n", item.uid);
...
void processCard(String uid) {
    ...
    if (pid == -1) { pid = 0; Serial.printf("⚠️ UID %s not mapped, using PID 0.\n", uid.c_str()); }

    if (sharedQueueA.exists(uid)) {
        sharedQueueA.removeByUID(uid);
        sharedQueueB.add(uid, timeStr, pid);
        Serial.printf("🔁 UID %s moved from SharedQueueA to SharedQueueB.\n", uid.c_str());
        sharedQueueA.print(); sharedQueueB.print();
    } else if (sharedQueue.exists(uid)) {
        Serial.printf("⏳ UID %s already in SharedQueue.\n", uid.c_str());
    } else if (sharedQueueB.exists(uid)) {
        Serial.printf("⏳ UID %s already in SharedQueueB.\n", uid.c_str());
    } else {
        sharedQueue.add(uid, timeStr, pid);
        Serial.printf("✅ UID %s added to SharedQueue.\n", uid.c_str());
        sharedQueue.print();
    }
}
...
void setup() {
    ...
    if (!rtc.begin()) { Serial.println("❌ Couldn't find RTC module!"); while (1); }
    if (rtc.lostPower()) {
    ...
    sharedQueueA.load(); Serial.println("📋 Queue A Ready."); sharedQueueA.print();
    sharedQueueB.load(); Serial.println("📋 Queue B Ready."); sharedQueueB.print();
}
