...
void onDataRecv(const esp_now_recv_info_t *recvInfo, const uint8_t *incomingData, int len) {
    ...
    if (sharedQueue.exists(entry.uid)) {
        sharedQueue.removeByUID(entry.uid);       // Remove original position
        sharedQueue.push(entry);                  // Push to back
    } else if (sharedQueueB.exists(entry.uid)) {
        sharedQueueB.removeByUID(entry.uid);
        sharedQueueB.push(entry);
    } else {
        // If not found in A or B, assume MixedQueue owns it fully
        sharedQueueC.push(entry);  // Fallback
    }
}
 else {
                QueueItem zeroItem = {};
...
void setup() {
    Serial.begin(115200);
    //clearAllQueues();  // Optional: clear queues at startup
...
void loop() {
    ...
    createMixedQueue(); printAllQueues();
}
