...
SharedQueue sharedQueueA("queue-A");  // 🔥 NEW queue
RTC_DS3231 rtc;
...
void onDataRecv(const esp_now_recv_info_t *recvInfo, const uint8_t *incomingData, int len) {
    ...
    if (isArrivalNode) {
        ...
        if (strcmp(item.uid, "REQ_NEXT") == 0) {
            ...
            sharedQueueA.print();  // Optional: Show Queue A status
        }
    } 
    else {
...
void setup() {
    ...
    sharedQueueA.load(); Serial.println("📋 Queue A Ready."); sharedQueueA.print();  // 🔥 Load queue A
}
